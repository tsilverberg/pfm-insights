...
const PocketIcon: React.FC<{ icon: string; bg: string; iconColor?: string }> = ({ icon, bg, iconColor }) => {
  const iconSvg: Record<string, React.ReactNode> = {
    ...
    travel: (
      <svg width="20" height="20" viewBox="0 0 19 20" fill="currentColor">
    ...
    savings: (
      <svg width="20" height="20" viewBox="0 0 19 18" fill="currentColor">
  ...
  return (
    <div className="pocket-card__icon" style={{ background: bg, color: iconColor ?? undefined }}>
    ...
    >
      <div className="pocket-card__header">
        <div className="pocket-card__info">
          <PocketIcon icon={pocket.icon} bg={pocket.iconBg} iconColor={pocket.progressColor} />
